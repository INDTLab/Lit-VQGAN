...
from basicsr.archs.sh7vanmul_arch import sh7vanmul
...
def main(args):
    ...
    # ***************************************************************************************
    # attention x2 or x4 is different
    model = sh7vanmul(n_feats=64, kernel_size=7, n_blocks=5, mlp_ratio=2, upscaling_factor=args.upscaling_factor)
    # ****************************************************************************************
    ...
    for img in util.get_image_paths(args.lr_path):
        ...
        save_img_path = os.path.join(save_path, img_name + '_SR' + ext)
        util.imsave(img_E, save_img_path)
        logger.info('Saved SR image: {}'.format(save_img_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='sh7vanmul', help='method name')
    parser.add_argument('--lr_path', type=str, default='datasets/val_best/', help='Path to the LR image')
    parser.add_argument('--log_path', type=str, default='sr_result/', help='Path to log file')
    ...
    parser.add_argument('--save_path', type=str, default='sr_result/', help='Path to results')
    parser.add_argument('--upscaling_factor', type=int, default=4, help='Upscaling factor')
    parser.add_argument('--pretrain_model', type=str, default='experiments/sh7vanmul_val4/models/net_g_1000000.pth', help='Path to the pretrained model')
